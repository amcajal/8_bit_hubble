...
*   File: gui.c
*
*   Description: Source file of GUI module for LINUX. It implements
...
struct gui_io_param {
	GtkEntry *img_name;
	GtkEntry *seed;
	GtkFileChooserButton *output_dir;
...
call_entry_point (gchar *text, struct gui_io_param *p_giop)
{
    ...
    /* Add 7 to ignore "file://" part of the URI. */
	char *output_dir = (gtk_file_chooser_get_uri((GtkFileChooser *)p_giop->output_dir))+7;
	store_output_path(output_dir);
    
    char *file_name = (char *)gtk_entry_get_text(p_giop->img_name);
	...
	if (app_return_code == NO_ERROR)
	{
        ...
        /* Add 6 chars: one for the slash between path and name, 
            four for the ".png", and other for \0" */
		char *full_path = malloc((strlen(file_name) + strlen(output_dir) + 6) 
			* sizeof(char));
		full_path = strcpy(full_path, output_dir);
...
{    
    ...
    output_dir = (GtkFileChooserButton *)gtk_file_chooser_button_new ("Select output directory:",
                             GTK_FILE_CHOOSER_ACTION_SELECT_FOLDER);
	gtk_file_chooser_set_current_folder((GtkFileChooser *)output_dir, "./");
    ...
    gtk_grid_attach (GTK_GRID (grid), (GtkWidget *)output_dir, 0, 2, 1, 1);
